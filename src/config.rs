...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            beep_volume: 0.1,
            // Local Whisper GPU defaults: use the GPU automatically when compiled with
            // the `vulkan` feature, otherwise fall back to CPU. Override with WHISPER_BACKEND.
            whisper_backend: if cfg!(feature = "vulkan") {
                "vulkan".to_string()
            } else {
                "cpu".to_string()
            },
